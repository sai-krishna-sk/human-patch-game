...
start_str = "  const WhatsAppApp = () => {\n"
"""We need the first occurrence of WhatsAppApp"""
start_idx = text.find(start_str)

end_str = "  // --- MAIN RENDERS ---"
...
replacement = """  // --- SHARED WHATSAPP HELPERS ---
        ...
        <div className="flex-1 flex flex-col bg-zinc-50 text-black pt-10">
          <div className="p-4 bg-zinc-100 border-b border-zinc-300 flex items-center justify-between">
            ...
            <span className="text-blue-500 font-bold cursor-pointer text-sm" onClick={() => {
              if (name.trim() !== '') {
                if (name.lower().includes('priya') && storyProgress === 6) {
...
new_text = text[:start_idx] + replacement + text[end_idx + len(end_str):]
